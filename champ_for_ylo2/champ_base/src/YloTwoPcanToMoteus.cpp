#include "YloTwoPcanToMoteus.hpp"
...
YloTwoPcanToMoteus::YloTwoPcanToMoteus()
{
  ...
  /* TX STOP PACKAGE --------------------------------------------------------------------------
  -> exemple of return from moteus frame message analys
  1
  01 - WRITE_REGISTERS - Type.INT8 1 registers
    00 - Starting at reg 0x000(MODE)
      00 - Reg 0x000(MODE) = 0(STOPPED)
  31
    1f - READ_REGISTERS - F32 3 registers // pos, vel, tor
  ...
  12
  0c - WRITE_REGISTERS - Type.F32 
    06 - 6 registers
      20 - Starting at reg 0x020(COMMAND_POSITION)
        0000c07f - Reg 0x020(COMMAND_POSITION) = nan
        00000000 - Reg 0x021(COMMAND_VELOCITY) = 0.0
        cdcc4c3f - Reg 0x022(COMMAND_FEEDFORWARD_TORQUE) = 0.800000011920929
        00000000 - Reg 0x023(COMMAND_KP_SCALE) = 0.0
        00000000 - Reg 0x024(COMMAND_KD_SCALE) = 0.0
        0000003f - Reg 0x025(COMMAND_POSITION_MAX_TORQUE) = 0.5
  31
  ...
  _tx_msg.DLC = 14; // 32 bytes ; 14 = 48 bytes
  // Write Mode
...
bool YloTwoPcanToMoteus::initialize(){
  ...
  for (unsigned int pp = 0; pp < 4; pp++){
    ...
    CAN_GetErrorText(Status, 0x0c, strMsg); // check the Status return state
	  if (Status){
    ...
    CAN_GetErrorText(Status, 0x0c, strMsg);
    if(Status){
...
bool YloTwoPcanToMoteus::send_pos(int id, int port, float position){
    ...
    std::cout << "erreur dans fonction send_pos(), id=" << id << ". Status = " << strMsg << std::endl;
    return(false);
}
